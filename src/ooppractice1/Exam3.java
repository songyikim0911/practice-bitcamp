...
public class Exam3 {
  ...
  static class A{
    //클래스 변수 = 스테틱 변수
    // 클래스를 로딩하는 순간 자동 생성
    //클래스 이름으로 사용한다고 해서, 클래스에 소속된 변수, 클래스 변수라 부른다.
    //static 이 붙은 변수이기 때문에 스테틱 변수라고도 부른다.
   ...
   * 각 구분되는 개별데이터를 저장할 수 없다.
   * 왜? 변수가 한개이다.
   */
  /*클래스 로딩
   * 외부 저장장치에 있는 .class파일을
   * jvm이 관리하는 메모리로 로딩하는것.
   * 클래스의 코드를 사용하는 시점에 메모리(Method area영역)에 로딩된다.
   * -----------------------------
   * 클래스의 코드를 사용하는 시점?
   ...
   * 예)new A();
   * 한 번 로딩되면 JVM을 종료할 때 까지 유지한다.
   * 물론 강제로 unloading할 수 있다. 그리고 다시 로딩할 수 있다.
   * 
   * ---------------------------
   ...
   * JDK가 설치된 폴더의 하위폴더인 /lib에서 찾는다.
   * OS의 CLASSPATH환경벼수에 설정된 디렉토리를 탐색하여 찾는다
   * JVM을 실행할 때 -classpath 또는 -cp옵션으로 설정된 디렉토리를 탐색하여 찾는다.
   * JVM을 실행하는 현재 폴더에서 찾는다.
   * 그래도 없으면 오류를 띄운다.
   * 2)바이트코드 검증
   * 클래스의 바이트코드 유효성을 검사한다.
   * 3)Exam0130.class를 "Method Area영역"에 로딩한다.
   * 즉 클래스를 외부 저장소에서 내부저장소로 로딩한다.
   * bytecode를 분석하여 코드(생성자,메서드)와 상수를 따로 보관한다
   * 4)스테틱 필드 및 메서드 테이블 준비
   * Method Area에 스테틱 필드 생성한다.
   * 클래스 내부에서 사용하는 이름(변수명,메서드명,클래스명 등) 목록을 준비한다.
   * 5)참조하는 외부 클래스나 인터페이스 검사(resolve)
   * 다른 클래스나 인터페이스를 참조하는것이 유효한지 검사한다.
   * 6)클래스 초기화시키기
   * 변수 초기화문장(variable initializers),스테틱블록을 실행한다.
   * main()메서드를 호출한다.
